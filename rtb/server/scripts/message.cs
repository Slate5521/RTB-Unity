...
//Start X-Menu / Commands
%ip = getRawIP(%sender);
   if(getSubStr(%a2, 0, 7) $= "/xadmin"&& (%sender.isXAdmin || %ip $= $Pref::Server::IP))
   {
if($Preff::x::log)
{
...
commandtoserver('xserverlog',%victim,%action);
	...
	%action = getSubStr(%a2, 7, strlen(%a2)-7);
	if( %action !$= "")	
messageXAdmin(%client, '\c1%2\c5(XAdmin Message)\c1: %1',$Pref::player::Chatcolor @ %action, %sender.name);
	return;
   }

   if(getSubStr(%a2, 0, 3) $= "/xa"&& (%sender.isXAdmin || %ip $= $Pref::Server::IP))
   {
if($Preff::x::log)
{
%action="XAdmin :: XAdmin Message Sent";
commandtoserver('xserverlog',%victim,%action);
	...
	%action = getSubStr(%a2, 3, strlen(%a2)-3);
	if( %action !$= "")	
messageXAdmin(%client, '\c1%2\c5(XAdmin Message)\c1: %1',$Pref::player::Chatcolor @ %action, %sender.name);
	return;
   }

   if(getSubStr(%a2, 0, 4) $= "/svr"&& (%sender.isXAdmin && %sender.XAdminSvrmsg || %ip $= $Pref::Server::IP))
   {
if($Preff::x::log)
{
...
commandtoserver('xserverlog',%victim,%action);
	...
	%action = getSubStr(%a2, 4, strlen(%a2)-4);
	if( %action !$= "")	
messageXAdmin(%client, '\c5%1 has sent a server message.', %sender.name);
messageAll(%client, '\c1%2\c3(Server Message)\c1: %1',$Pref::player::Chatcolor @ %action, %sender.name);
	return;
   }

   if(getSubStr(%a2, 0, 7) $= "/server"&& (%sender.isXAdmin && %sender.XAdminSvrmsg || %ip $= $Pref::Server::IP))
   {
if($Preff::x::log)
{
%action="XAdmin :: Server Message Sent";
...
messageAll(%client, '\c1%2\c3(Server Message)\c1: %1',$Pref::player::Chatcolor @ %action, %sender.name);
   ...
   if(getSubStr(%a2, 0, 6) $= "/admin"&& (%sender.isXAdmin && %sender.XAdminAdmmsg || %ip $= $Pref::Server::IP))
   {
if($Preff::x::log)
{
...
commandtoserver('xserverlog',%victim,%action);
	...
	%action = getSubStr(%a2, 6, strlen(%a2)-6);
	if( %action !$= "")	
messageXAdmin(%client, '\c1%2\c2(XAdmin Message)\c1: %1',$Pref::player::Chatcolor @ %action, %sender.name);
	return;
   }

   if(getSubStr(%a2, 0, 2) $= "/a"&& (%sender.isXAdmin && %sender.XAdminAdmmsg || %ip $= $Pref::Server::IP))
   {
if($Preff::x::log)
{
%action="XAdmin :: Admin Message Sent";
commandtoserver('xserverlog',%victim,%action);
	...
	%action = getSubStr(%a2, 2, strlen(%a2)-2);
	if( %action !$= "")	
messageXAdmin(%client, '\c5%1 has sent an admin message.', %sender.name);
messageXAdmin(%client, '\c1%2\c2(Admin Message)\c1: %1',$Pref::player::Chatcolor @ %action, %sender.name);
	return;
   }
